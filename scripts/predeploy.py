...
from models import User, Letter, Response, PhysicalMailbox, Post, Event, UserRole  # type: ignore
...
def migrate_sqlite_to_postgres_if_needed() -> None:
    ...
    sqlite_path = os.path.join(os.path.dirname(__file__), '..', 'NPO-SCA', 'instance', 'database.db')
    ...
    with closing(sqlite3.connect(sqlite_path)) as conn:
        ...
        # Physical mailboxes
        pm_table = 'physical_mailbox' if table_exists('physical_mailbox') else ('physical_mailboxes' if table_exists('physical_mailboxes') else None)
        if pm_table:
            rows = read_all(pm_table)
            for r in rows:
                try:
                    pm = PhysicalMailbox(
                        id=r.get('id'),
        ...
        def bump_sequence(table: str):
            try:
                ...
                # Fallback with explicit quoting
                try:
                    db.session.execute(text(
...
def run():
    ...
    with app.app_context():
        ...
        # One-time data migration when moving to Postgres
        try:
